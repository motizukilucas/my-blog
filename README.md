...
## Backup
sudo docker exec [CONTAINER_ID] /usr/bin/mysqldump -u root --password=root [DATABASE_NAME] > backup.sql
> [CONTAINER_ID] = mysql container id
> [DATABASE_NAME] = wordpress if unchanged

## Restore
cat backup.sql | sudo docker exec -i [CONTAINER_ID] /usr/bin/mysql -u root --password=root [DATABASE_NAME]
